...
def main() -> None:
    parser = argparse.ArgumentParser(description="Run CO3Dv2 high-resolution Ours diagnostics and report.")
    ...
    parser.add_argument("--out", default="outputs/co3dv2_highres_diagnostics")
    ...
    if args.strict_ready and not ready["ready"]:
        raise SystemExit(1)
    run_ours_benchmark(args.manifest, cfg, args.out, max_images=args.max_images, use_frontend_cache=True, frontend_cache_manifest=args.frontend_cache_manifest)
    if args.generate_report:
        generate_co3dv2_highres_report(args.out, Path(args.out) / "report")
    print(f"co3dv2 highres diagnostics saved to: {Path(args.out).resolve()}")
