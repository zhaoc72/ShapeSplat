...
def generate_final_report(final_root: str | Path, out_dir: str | Path, title: str = "ShapeSplat++ Final Paper Report") -> dict:
    ...
    lines += ["", "## Notes", "- `final_debug` is a smoke test, not a submission result.", "- Geometry metrics are only shown when pred/GT pointclouds exist.", "- Missing real external baselines are readiness warnings, not runtime failures."]
