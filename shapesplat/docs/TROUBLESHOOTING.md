...
```
## CO3Dv2 output still looks low resolution

Symptoms:

- `input.png`, `masks.png`, `render_final.png`, `alpha_final.png`, or ownership maps look close to `128x128`.
- CO3Dv2 benchmark validation reports original images around `640x479`.

Likely causes:

- The run used `configs/final_ours.yaml` or another minimal square-resize config.
- The frontend cache was generated from a low-resolution config.
- File masks were resized through a non-nearest path.
- Debug iteration cap was applied.
...
- CO3Dv2 main diagnostics use `mask_source: file`; SAM3 is only optional automatic-mask diagnostic.
- Report thumbnails are not full-resolution proof. Inspect each per-image directory for `input_fullres_copy.png`, `input_working.png`, and `masks_working.png`.
- SoftGaussianRenderer and ToyShapeBank fallback are diagnostic-only and should not be described as final paper quality.
