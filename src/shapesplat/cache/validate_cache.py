...
def validate_frontend_cache_dir(cache_dir: str | Path, image_hw: tuple[int, int] | None = None) -> dict:
    ...
    if not errors:
        try:
            ...
            meta = json.loads((root / "frontend_meta.json").read_text(encoding="utf-8"))
            if masks.ndim != 3:
            ...
            working = meta.get("working_image_shape")
            if working and height is not None:
                expected_hw = tuple(int(v) for v in working[-2:])
                if (height, width) != expected_hw:
                    errors.append(f"cache HW {(height, width)} != metadata working HW {expected_hw}")
            if height is not None and (height <= 160 or width <= 160):
                warnings.append("cache appears low resolution")
...
def validate_frontend_cache_manifest(cache_manifest: str | Path) -> dict:
    ...
    if len(rows) == 0:
        warnings.append("cache manifest has zero records")
        warnings.append("cache manifest has zero valid records")
    elif num_valid == 0:
        warnings.append("cache manifest has zero valid records")
        if num_failed == len(rows):
            errors.append("all cache records failed validation")
    return {
        "valid": len(rows) > 0 and all(r["valid"] for r in rows),
